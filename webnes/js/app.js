...
(function () {
	...
	var App = function () {
		...
		window.onerror = function (e) { that.showError(e); };
	};
	...
	App.prototype.loadRomCallback = function (name, binaryString) {
	...
	App.prototype.start = function (options) {
		...
		if (this.options.createGuiComponents) {
			...
			this.fpsMeter.hide();
			Gui.hookDragDropEvents(function (name, binaryString) { that.loadRomCallback(name, binaryString); });
		...
		this.mainboard.connect('reset', function () { that.onReset(); });

		if (this.options.createGuiComponents) {
			this.ggDialog = new Gui.GameGenieDialog(this);
			this.controlBar = new Gui.ControlBar(this);
			this.controlBar.connect('romLoaded', function (name, binaryString) { that.loadRomCallback(name, binaryString); });
	...
	App.prototype.calculateFrameTimeTarget = function () {
		if (this.gameSpeed > 0) {
	...
	App.prototype.readyToRender = function () {
		if (this.gameSpeed <= 0) {
			return true;
		}
	...
	App.prototype.doRomLoad = function (name, binaryString) {
		var that = this;
	...
	App.prototype.showError = function (err) {
