...
export default class Mapper5 extends BaseMapper {
  ...
  mapperSaveState(state) {
    ...
    state._prgRegisters = uintArrayToString(this._prgRegisters);
    state._nameTableFill = uintArrayToString(this._nameTableFill);
    state._internalExRam = uintArrayToString(this._internalExRam);
    state._prgRam = uintArrayToString(this._prgRam);
    ...
    state._prgRamMap = uintArrayToString(this._prgRamMap);
    state._prgRamIsActive = uintArrayToString(this._prgRamIsActive);
    state._nameTableMap = uintArrayToString(this._nameTableMap);

    state._chrRegsA = uintArrayToString(this._chrRegsA);
    state._chrRegsB = uintArrayToString(this._chrRegsB);

    state._chrUseBMap = this._chrUseBMap;
    state._chrMapA = uintArrayToString(this._chrMapA);
    state._chrMapB = uintArrayToString(this._chrMapB);
  ...
  reset() {
    ...
    const that = this;
    this._irqEventId = this.mainboard.synchroniser.addEvent('mmc5 irq', -1, (eventTime) => { that._irqEvent(eventTime); });
  }
  ...
  _syncPrg() {
    ...
    for (let i = 0; i < this._prgRamMap.length; ++i) {
      this._prgRamMap[i] = 0;
      this._prgRamIsActive[i] = 0;
    }
  ...
  _predictIrq(cpuMTC) {
    ...
    if (this.mRenderingEnabled && !this._irqActive && this._irqEnabled && this._irqScanlineTrigger > 0) {
      const targetScanline = this._irqScanlineTrigger;
