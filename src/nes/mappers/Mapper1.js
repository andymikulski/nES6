...
export default class Mapper1 extends BaseMapper {
  ...
  syncPrgMirrors() {
  	const offset = this.soromlatch ? 16 : 0; // if more than 256k ROM AND SOROM latch is on
  	const reg = this.registers[3];
  	if ((this.registers[0] & 0x8) > 0) { // 16k / 32k prg switch
  		if ((this.registers[0] & 0x4) > 0) { // high/low prg switch
  			const last = Math.min(this.get16kPrgBankCount() - 1, 15); // this.get16kPrgBankCount() - 1;
  ...
  write8PrgRom(offset, data) {
  	...
  	const currTime = this.mainboard.synchroniser.getCpuMTC();
  	const minTime = this.lastWriteMTC + (COLOUR_ENCODING_MTC_PER_CPU * 2);
  	const valid = this.lastWriteMTC >= 0;
