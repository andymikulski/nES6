...
export default class Input {
	constructor( mainboard ) {
		...
		if ( this._gamepadsSupported ) {
			...
			window.addEventListener('gamepadconnected', ::this._populateGamepads, true);
			window.addEventListener('gamepaddisconnected', ::this._populateGamepads, true);
		}
	}
...
//					this._doGamepadAxis( i, axisIndex, axisState === 1, pads[ i ].axes[ axisIndex ] > 0 ? 'positive' : 'negative' );
	...
	_doGamepadAxis( gamepadIndex, axisIndex, isPressed, axisType ) {
	...
	_doGamepadButton( gamepadIndex, buttonIndex, isPressed ) {
		var joypad = this._mainboard.inputdevicebus.getJoypad( gamepadIndex );
	...
	_doKeyboardButtonPress( keyCode, pressed ) {
	...
	saveKeyBindings( playerId, key, keysAssigned ) {
	...
	getKeyBindings( playerId, key ) {

		return this._playerKeyboardMaps[ playerId ][ key ].slice( 0 );
	}


	_loadKeyBindingsFromLocalStorage() {
		this._playerKeyboardMaps = loadFromLocalStorage( "webnes_keybindings" );
	...
	_saveKeyBindingsToLocalStorage() {
		saveToLocalStorage( "webnes_keybindings", this._playerKeyboardMaps );
	}
}
